...
[ApiController]
[Route("api/obs")]
public sealed class OBSController(ICaramelServiceClient caramelClient) : ControllerBase
{
  [HttpGet("status")]
  ...
  public async Task<IActionResult> SetSceneAsync([FromBody] SetSceneRequest request, CancellationToken cancellationToken)
  {
    ...
    return result.IsFailed
      ? StatusCode(StatusCodes.Status502BadGateway, result.Errors.Select(e => e.Message))
      : Ok(result.Value);
  }
}
